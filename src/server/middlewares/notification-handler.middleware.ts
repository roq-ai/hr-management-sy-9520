...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  ...
  'user.delete': {
    ...
    key: 'user-account-deactivated',
    tenantPath: ['organization', 'user'],
    userPath: [],
  },
  'organization.update': { roles: ['hr-manager'], key: 'data-backup', tenantPath: ['organization'], userPath: [] },
};

const ownerRoles: string[] = ['hr-manager'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['employee', 'payroll-officer', 'team-lead', 'hr-manager', 'system-administrator'];
